...
class WxPayApi
{
    ...
    /**
     * @param $inputObj WxPayAppPay
     * @throws WxPayException
     * @return 成功时返回，其他抛异常
     */
    public function appPay($inputObj)
    {
        //检测必填参数
        if (!$inputObj->IsPrepayidSet()) {
            throw new WxPayException("缺少统一支付接口必填参数prepayid！");
        } else if (!$inputObj->IsPackageSet()) {
            throw new WxPayException("缺少统一支付接口必填参数package！");
        }
    ...
    private function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        if ($useCert == true) {
            ...
            curl_setopt($ch, CURLOPT_SSLKEY, $this->config->SSLKEY);
        }
        //post提交方式
