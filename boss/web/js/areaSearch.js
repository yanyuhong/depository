...
function initAreaSearch(provinceBox, cityBox, areaBox, province, city, area) {
    areaSearch.text.province = province;
    areaSearch.text.city = city;
    areaSearch.text.area = area;
    if ($('#' + provinceBox).length > 0) {
        areaSearch.box.province = $('#' + provinceBox);
        if ($('#' + cityBox).length > 0) {
            areaSearch.box.city = $('#' + cityBox);
            areaSearch.box.province.change(function () {
                getCities(areaSearch.box.province.val(), '');
            });
            if ($('#' + areaBox).length > 0) {
                areaSearch.box.area = $('#' + areaBox);
                areaSearch.box.city.change(function () {
                    //getCities(cityBox, city);
                });
            }
        }
        getProvinces(areaSearch.text.province);
    }
}

function getProvinces(province) {
    areaSearch.box.province.empty().append('<option value="">请选择省份</option>');
    $.each(data, function (index, element) {
        var dplacehoder = '';
        if (province != '' && province == element.name) {
            dplacehoder = 'data-placeholder="' + element.name + '" selected';
            getCities(element.name, areaSearch.text.city);
        }
        areaSearch.box.province.append('<option value="' + element.name + '" ' + dplacehoder + '>' + element.name + '</option>');
    });
}

function getCities(province, city) {
    areaSearch.box.city.empty().append('<option value="">请选择城市</option>');
    $.each(data, function (index, element) {
        if (province != '' && province == element.name) {
            $.each(element.city, function (index, element) {
                var dplacehoder = '';
                if (city != 0 && city == element.name) {
                    dplacehoder = 'data-placeholder="' + element.name + '" selected';
                }
                areaSearch.box.city.append('<option value="' + element.name + '"' + dplacehoder + '>' + element.name + '</option>');
            });
            return;
        }
    });
...
var areaSearch = {};
areaSearch.box = {province: null, city: null, area: null};
areaSearch.text = {province: null, city: null, area: null};
